...
public class UppaalModelExport implements ExportTool {
	...
	public int exportToXML(Block model, String exportFile, int abstractionDepth) {
		...
		String outputFolder = exportFile.replace("uppaal.xml", "");

		if (model.getClass() == SourceBlock.class) {
			for (Block methodBlock : model.getChildBlocks()) {
				try {
					uppaalModel.add(createTimedAutomaton(methodBlock, abstractionDepth, outputFolder));
				} catch (Exception e) {
			...
			try {
				uppaalModel.add(createTimedAutomaton(model, abstractionDepth, outputFolder));
			} catch (Exception e) {
				System.err.println("Failed to export model to XML!\n" + e.getMessage());
				e.printStackTrace();
	...
	public TimedAutomaton createTimedAutomaton(Block model, int abstractionDepth, String outputFolder)
			throws Exception {
		TimedAutomaton automaton = new TimedAutomaton();

		automaton.createAutomaton(model, abstractionDepth, outputFolder);
			...
			for (TimedAutomaton system : systems) {
				...
				for (Node node : system.getNodes()) {
					String nodeString = "<location id=\"id" + node.getId() + "\" x=\"" + node.getLocX() + "\" y=\""
							...
							+ (node.getInvariant() != null
									? (node.getName().matches("r" + "\\d+.*") ? node.getInvariant() : "") : "")
							+ "</label>" + "<label kind=\"comments\">" + formatXMLString(node.getComments())
							+ "</label>" + (node.isCommitted() ? "<committed/>" : "") + "</location>" + eol;

...
public class TimedAutomaton {
	...
	public int createAutomaton(Block model, int abstractionDepth, String outputFolder) throws Exception {

		Vector<Node> endNodes = new Vector<Node>();
		Vector<Node> condNodes = new Vector<Node>();
		if (model.getClass() != MethodBlock.class) {
			throw new Exception("Top block needs to be from the class MethodBlock!" + model.getClass() + "XXXX "
					+ MethodBlock.class);
		}
		// Set automaton name
		...
		genericModelTransformation(this.nodes, this.name, outputFolder);
	...
	private void genericModelTransformation(Vector<Node> nodes, String name, String outputFolder) throws IOException {
		...
		for (Node itnode : nodes) {
			...
			if (itnode.getName().matches("r" + "\\d+.*")) {
				...
				localVariablesWCET = localVariablesWCET + itnode.getName() + "_WCET=, ";
			}
		}
		...
		String WCETfilename = outputFolder + this.name + ".txt";
		File file;
		// Read file
		try {
			file = new File(WCETfilename);

			if (file.exists()) {
				FileReader reader = new FileReader(file.getAbsoluteFile());
				BufferedReader buffReader = new BufferedReader(reader);
				String WCETs = buffReader.readLine();
				if (WCETs!=null) {
					setLocalVariables(WCETs + eol);
				}
				buffReader.close();
			}
		} catch (IOException e) {
			// Could not create file
			throw new IOException("Could not open file: " + WCETfilename + "\nMessage: " + e.getMessage());
		}

		String WCETfilenameExport = outputFolder + "WCETfiles/" + this.name + ".txt";

		exportRequiredWCETfiles(WCETfilenameExport, localVariablesWCET, outputFolder);
	}

	private void exportRequiredWCETfiles(String WCETfilenameExport, String localVariablesWCET, String outputFolder)
			throws IOException {
		File WCETfilesFolder = new File(outputFolder + "WCETfiles/");
		WCETfilesFolder.mkdirs();

		File file = new File(WCETfilenameExport);
		...
		try {
			...
			buffWriter.write(localVariablesWCET);
			buffWriter.close();
		} catch (IOException e) {
	...
	private Vector<Node> functionCallCheck(Node node, int startRowNumber) {
		...
		if ((node.getComments().length() - node.getComments().replace(");", "").length())
			...
			String[] Chan = node.getComments().replaceAll("[\n\\s]", "").startsWith("(")
					? node.getComments().replaceAll("[\n\\s]", "").replaceFirst("\\)", "_replacemarkingpinxxxx")
							.replaceAll(".*_replacemarkingpinxxxx", "").split("\\)" + ";")
					: node.getComments().replaceAll("[\n\\s]", "").split("\\)" + ";");
	...
	private Vector<Node> createIterationNode(IterationBlock iterationBlock, Vector<Node> parentNodes,
		...
		if (leftAbstractionLevel == 0) {
			...
			// creat the node of the iteration block
			Node nodeprime = new Node();
			nodeprime.setId(getNextFreeId());
			nodeprime.setName("r" + iterationBlock.getStartRowNumber());
			nodeprime.setComments(iterationBlock.getCodeSegment().toString());

			this.nodes.add(nodeprime);

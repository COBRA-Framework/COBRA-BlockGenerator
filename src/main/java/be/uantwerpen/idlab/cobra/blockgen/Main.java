...
import be.uantwerpen.idlab.cobra.blockgen.tools.exporting.blockmodel.BlockModelExport;
...
public class Main {
		...
		if (!terminalMode) {
			try {
				...
				if (showGraph) {
					...
					Terminal.printTerminalInfo(
							"Block Viewer is ready. Close the Block Viewer window or use 'ctrl+c' in the console to terminate the application.");
				}
			} catch (Exception e) {
	...
	private static SourceBlock runBlockGenerator(SourceFile file, String exportLocation, Grammar grammar)
			throws Exception {
		...
		for (Block methodBlock : sourceBlock.getChildBlocks()) {
			BlockGenerationService.applyAbstractionRuleRecursive(methodBlock,3);
			BlockGenerationService.applyReductionRuleRecursive(methodBlock);
			
			for (Block leafBlock : methodBlock.getLeafs()) {
				benchmarkExport.generateSourceOfBlock(
						exportLocation + "HPA_source" + File.separator + ((MethodBlock) methodBlock).getName() + "_",
						leafBlock, grammar);
			}
		}
